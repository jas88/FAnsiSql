...
internal abstract class TableHelperUpdateTestsBase : DatabaseTests
{
    ...
    protected void UpdateWithJoin_SelfJoin_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            {
                new(
                    $"t1.{syntaxHelper.EnsureWrapped("Value")} = t1.{syntaxHelper.EnsureWrapped("Value")} + t2.{syntaxHelper.EnsureWrapped("Value")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("ParentId")} = t2.{syntaxHelper.EnsureWrapped("Id")}",
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 3",
                       con))
            {
    ...
    protected void UpdateWithJoin_MultipleJoinConditions_JoinsCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Score")} = t2.{syntaxHelper.EnsureWrapped("NewScore")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("FirstName")} = t2.{syntaxHelper.EnsureWrapped("First")}",
                    QueryComponent.JoinInfoJoin),
                new($"t1.{syntaxHelper.EnsureWrapped("LastName")} = t2.{syntaxHelper.EnsureWrapped("Last")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("FirstName")} = 'John' AND {syntaxHelper.EnsureWrapped("LastName")} = 'Doe'",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("FirstName")} = 'John' AND {syntaxHelper.EnsureWrapped("LastName")} = 'Smith'",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("FirstName")} = 'Jane' AND {syntaxHelper.EnsureWrapped("LastName")} = 'Doe'",
                       con))
            {
    ...
    protected void UpdateWithJoin_SpecialCharactersInData_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Text")} = t2.{syntaxHelper.EnsureWrapped("NewText")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("TextId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Text")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Text")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
    ...
    protected void UpdateWithJoin_StringConcatenation_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            if (dbType == DatabaseType.MicrosoftSQLServer)
                concatExpression =
                    $"t2.{syntaxHelper.EnsureWrapped("FirstName")} + ' ' + t2.{syntaxHelper.EnsureWrapped("LastName")}";
            else if (dbType == DatabaseType.MySql)
                // MySQL requires CONCAT() function as || is logical OR by default
                concatExpression =
                    $"CONCAT(t2.{syntaxHelper.EnsureWrapped("FirstName")}, ' ', t2.{syntaxHelper.EnsureWrapped("LastName")})";
            else
            ...
            var queryLines = new List<CustomLine>
            {
                ...
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("PersonId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("FullName")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("FullName")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
    ...
    protected void UpdateWithJoin_OrConditionInWhere_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Flag")} = t2.{syntaxHelper.EnsureWrapped("NewFlag")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("CatId")}",
                    QueryComponent.JoinInfoJoin),
                new(
                    $"t1.{syntaxHelper.EnsureWrapped("Category")} = 'A' OR t1.{syntaxHelper.EnsureWrapped("Category")} = 'B'",
            ...
            // Verify updates
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Flag")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Flag")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Flag")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 3",
                       con))
            {
    ...
    protected void UpdateWithJoin_LessThanComparison_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("CurrentValue")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("CompId")}",
                    QueryComponent.JoinInfoJoin),
                new($"t1.{syntaxHelper.EnsureWrapped("OldValue")} < t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.WHERE),
                new($"t2.{syntaxHelper.EnsureWrapped("NewValue")} < t1.{syntaxHelper.EnsureWrapped("CurrentValue")}",
                    QueryComponent.WHERE)
            };
            ...
            // Verify updates
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("CurrentValue")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("CurrentValue")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
    ...
    protected void UpdateWithJoin_BooleanValues_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("IsActive")} = t2.{syntaxHelper.EnsureWrapped("NewActive")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("FlagId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("IsActive")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("IsActive")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
                var result = cmd.ExecuteScalar();
    ...
    protected void UpdateWithJoin_InOperator_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Priority")} = t2.{syntaxHelper.EnsureWrapped("NewPriority")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("TaskId")}",
                    QueryComponent.JoinInfoJoin),
            ...
            // Verify updates
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Priority")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Priority")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Priority")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 3",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Priority")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 4",
                       con))
            {
    ...
    protected void Test_UpdateTableFromJoin_OriginalTest(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            queryLines.Add(new CustomLine($"t1.{highScore} < t2.{score} OR t1.{highScore} is null",
                QueryComponent.WHERE));
    ...
    protected void UpdateWithJoin_SingleColumn_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Score")} = t2.{syntaxHelper.EnsureWrapped("NewScore")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Name")} = t2.{syntaxHelper.EnsureWrapped("Name")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Name")} = 'Alice'",
                       con))
            {
                Assert.That(cmd.ExecuteScalar(), Is.EqualTo(150));
            }
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Name")} = 'Bob'",
                       con))
            {
                Assert.That(cmd.ExecuteScalar(), Is.EqualTo(250));
            }
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Score")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Name")} = 'Charlie'",
                       con))
            {
    ...
    protected void UpdateWithJoin_MultipleColumns_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Name")} = t2.{syntaxHelper.EnsureWrapped("NewName")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Age")} = t2.{syntaxHelper.EnsureWrapped("NewAge")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("PersonId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Name")}, {syntaxHelper.EnsureWrapped("Age")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
    ...
    protected void UpdateWithJoin_WithWhereClause_FiltersCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("NumId")}",
                    QueryComponent.JoinInfoJoin),
            ...
            // Verify updates
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 3",
                       con))
            {
    ...
    protected void UpdateWithJoin_MultipleWhereConditions_AppliesAllConditions(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("RecordId")}",
                    QueryComponent.JoinInfoJoin),
    ...
    protected void UpdateWithJoin_SetToNull_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("DataId")}",
                    QueryComponent.JoinInfoJoin)
            };
    ...
    protected void UpdateWithJoin_WhereIsNull_FiltersCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("TestId")}",
                    QueryComponent.JoinInfoJoin),
    ...
    protected void UpdateWithJoin_DateTimeValues_UpdatesCorrectly(DatabaseType dbType)
    {
        if (dbType == DatabaseType.Sqlite)
            Assert.Ignore(
                "SQLite stores DateTime as TEXT strings; ADO.NET returns string type. Fundamental SQLite limitation.");
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("EventDate")} = t2.{syntaxHelper.EnsureWrapped("NewDate")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("EventId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("EventDate")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
                var result = cmd.ExecuteScalar();
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("EventDate")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
                var result = cmd.ExecuteScalar();
                Assert.That(((DateTime)result).ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
    ...
    protected void UpdateWithJoin_DecimalValues_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Price")} = t2.{syntaxHelper.EnsureWrapped("NewPrice")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("ProductId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Price")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Price")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
                var result = Convert.ToDecimal(cmd.ExecuteScalar(), CultureInfo.InvariantCulture);
    ...
    protected void UpdateWithJoin_NoMatchingRows_UpdatesNothing(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("DifferentId")}",
                    QueryComponent.JoinInfoJoin)
            };
    ...
    protected void UpdateWithJoin_EmptyUpdateTable_UpdatesNothing(DatabaseType dbType)
    {
        ...
        TestContext.Out.WriteLine(
            $"DEBUG tbl2 actual columns: {string.Join(", ", tbl2Cols.Select(c => $"{c.GetRuntimeName()}={c.DataType?.SQLType}"))}");
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("MainId")}",
                    QueryComponent.JoinInfoJoin)
            };
    ...
    protected void UpdateWithJoin_LargeDataSet_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            var queryLines = new List<CustomLine>
            {
                new($"t1.{syntaxHelper.EnsureWrapped("Value")} = t2.{syntaxHelper.EnsureWrapped("NewValue")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("LargeId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 500",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Value")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 501",
                       con))
            {
    ...
    protected void UpdateWithJoin_CalculatedExpression_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            {
                new(
                    $"t1.{syntaxHelper.EnsureWrapped("Total")} = t1.{syntaxHelper.EnsureWrapped("Quantity")} * t2.{syntaxHelper.EnsureWrapped("Price")}",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("OrderId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            // Verify calculations
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Total")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Total")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
    ...
    protected void UpdateWithJoin_CaseWhenExpression_UpdatesCorrectly(DatabaseType dbType)
    {
        ...
        try
        {
            ...
            {
                new(
                    $"t1.{syntaxHelper.EnsureWrapped("Status")} = CASE WHEN t2.{syntaxHelper.EnsureWrapped("Score")} >= 80 THEN 'High' ELSE 'Low' END",
                    QueryComponent.SET),
                new($"t1.{syntaxHelper.EnsureWrapped("Id")} = t2.{syntaxHelper.EnsureWrapped("ItemId")}",
                    QueryComponent.JoinInfoJoin)
            };
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Status")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 1",
                       con))
            {
            ...
            using (var cmd = db.Server.GetCommand(
                       $"SELECT {syntaxHelper.EnsureWrapped("Status")} FROM {tbl1.GetFullyQualifiedName()} WHERE {syntaxHelper.EnsureWrapped("Id")} = 2",
                       con))
            {
