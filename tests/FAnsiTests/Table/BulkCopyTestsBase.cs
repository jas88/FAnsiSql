...
/// <summary>
///     Abstract base class providing comprehensive test methods for BulkCopy implementations across all database
///     providers.
///     Tests cover basic operations, error handling, column mapping, transactions, and special data types.
///     This class is inherited by database-specific test classes that call these protected methods with specific
///     DatabaseType values.
/// </summary>
internal abstract class BulkCopyTestsBase : DatabaseTests
{
    ...
    protected void Upload_NullValues_InsertedCorrectly(DatabaseType type)
    {
        ...
        [
            ...
            new DatabaseColumnRequest("NullableString", new DatabaseTypeRequest(typeof(string), 50))
                { AllowNulls = true },
    ...
    protected void Upload_DecimalOutOfRange_ThrowsException(DatabaseType type)
    {
        ...
        [
            new DatabaseColumnRequest("Id", new DatabaseTypeRequest(typeof(int))),
            new DatabaseColumnRequest("SmallDecimal",
                new DatabaseTypeRequest(typeof(decimal), null, new DecimalSize(3, 2)))
        ]);
    ...
    protected void Upload_ViolateNotNullConstraint_ThrowsException(DatabaseType type)
    {
        ...
        [
            new DatabaseColumnRequest("Id", new DatabaseTypeRequest(typeof(int))) { AllowNulls = false },
            new DatabaseColumnRequest("RequiredValue", new DatabaseTypeRequest(typeof(string), 50))
                { AllowNulls = false }
        ]);
    ...
    protected void Upload_WithTransaction_CommitsProperly(DatabaseType type)
    {
        ...
        if (type == DatabaseType.MicrosoftSQLServer)
        {
            ...
            checkCon.Open();
            using var cmd = tbl.Database.Server.GetCommand(
                "SELECT CASE transaction_isolation_level WHEN 0 THEN 'Unspecified' WHEN 1 THEN 'ReadUncommitted' WHEN 2 THEN 'ReadCommitted' WHEN 3 THEN 'Repeatable' WHEN 4 THEN 'Serializable' WHEN 5 THEN 'Snapshot' END FROM sys.dm_exec_sessions WHERE session_id = @@SPID",
                checkCon);
    ...
    protected void Upload_DateTimeValues_PreservedCorrectly(DatabaseType type)
    {
        ...
        {
            Assert.That(
                ((DateTime)result.Rows[0]["EventDate"]).ToString("yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
    ...
    protected void Upload_DecimalExceedsPrecision_ThrowsException(DatabaseType type)
    {
        ...
        AssertThrowsException(type, () => bulk.Upload(dt),
            type == DatabaseType.Sqlite ? null : "1000",
            true);
    }
    ...
    protected void Upload_DecimalExceedsScale_ThrowsException(DatabaseType type)
    {
        ...
        AssertThrowsException(type, () => bulk.Upload(dt),
            type == DatabaseType.Sqlite ? null : "99.999",
            true);
    }
    ...
    protected void Upload_DecimalLargePrecisionViolation_ThrowsException(DatabaseType type)
    {
        ...
        [
            new DatabaseColumnRequest("Id", new DatabaseTypeRequest(typeof(int))),
            // decimal(10,4) = 6 digits before decimal + 4 digits after = DecimalSize(6, 4)
            new DatabaseColumnRequest("BigNumber",
                new DatabaseTypeRequest(typeof(decimal), null, new DecimalSize(6, 4)))
        ]);
        ...
        AssertThrowsException(type, () => bulk.Upload(dt),
            type == DatabaseType.Sqlite ? null : "9999999",
            true);
    }
    ...
    protected void Upload_DecimalZeroScale_Success(DatabaseType type)
    {
        ...
        [
            new DatabaseColumnRequest("Id", new DatabaseTypeRequest(typeof(int))),
            new DatabaseColumnRequest("IntegerOnly",
                new DatabaseTypeRequest(typeof(decimal), null, new DecimalSize(5, 0)))
        ]);
    ...
    protected void Upload_DecimalZeroScale_WithDecimalPlaces_ThrowsException(DatabaseType type)
    {
        ...
        AssertThrowsException(type, () => bulk.Upload(dt),
            type == DatabaseType.Sqlite ? null : "123.45",
            true);
    }
    ...
    protected void Upload_DecimalNullValues_IgnoredInValidation(DatabaseType type)
    {
        ...
        [
            new DatabaseColumnRequest("Id", new DatabaseTypeRequest(typeof(int))),
            // decimal(5,2) = 3 digits before decimal + 2 digits after = DecimalSize(3, 2)
            new DatabaseColumnRequest("NullableAmount",
                new DatabaseTypeRequest(typeof(decimal), null, new DecimalSize(3, 2)))
            {
                AllowNulls = true
            }
