        ...
        if (args.DataTable != null)
        {
            ...
            foreach (DataColumn column in args.DataTable.Columns)
            {
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine($"DEBUG EmptyTable: Column {column.ColumnName} has {column.Table?.Rows.Count ?? -1} rows, overriding Guesser type {guesser.Guess.CSharpType} with DataColumn type {column.DataType}");
