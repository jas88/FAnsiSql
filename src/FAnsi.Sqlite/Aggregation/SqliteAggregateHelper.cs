...
public sealed class SqliteAggregateHelper : AggregateHelper
{
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>Day: datetime(column, 'start of day')</description>
    ///         </item>
    ///         <item>
    ///             <description>Month: strftime('%Y-%m', column)</description>
    ///         </item>
    ///         <item>
    ///             <description>Year: CAST(strftime('%Y', column) AS INTEGER)</description>
    ///         </item>
    ///         <item>
    ///             <description>Quarter: Calculated from month using expression</description>
    ///         </item>
    ///     </list>
    /// </remarks>
    public override string GetDatePartOfColumn(AxisIncrement increment, string columnSql)
    {
        return increment switch
        {
            ...
            AxisIncrement.Quarter =>
                $"strftime('%Y', {columnSql}) || 'Q' || CAST(((CAST(strftime('%m', {columnSql}) AS INTEGER) - 1) / 3 + 1) AS TEXT)",
    ...
    /// </remarks>
    protected override string BuildPivotAndAxisAggregate(AggregateCustomLineCollection query) => throw
        ...
        new NotSupportedException(
            "SQLite does not support complex pivot operations with axis aggregates. Consider using simpler aggregation methods.");
    ...
    protected override string BuildPivotOnlyAggregate(AggregateCustomLineCollection query, CustomLine nonPivotColumn) =>
        throw
            ...
            // We would need to build this manually with CASE statements
            new NotSupportedException(
                "SQLite does not support complex pivot operations. Consider using GROUP BY with CASE statements for simple pivoting.");
}
