...
public sealed class SqliteServerHelper : DiscoveredServerHelper
{
    ...
    public override Dictionary<string, string> DescribeServer(DbConnectionStringBuilder builder)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(version)) toReturn.Add("SQLite Version", version);
        }
    ...
    public override DbCommand GetCommand(string s, DbConnection con, DbTransaction? transaction = null) =>
        new SqliteCommand(s, (SqliteConnection)con, transaction as SqliteTransaction);
    ...
    public override DbCommandBuilder GetCommandBuilder(DbCommand cmd) =>
        throw new NotSupportedException("SQLite does not support DbCommandBuilder directly");
    ...
    public override DbConnection GetConnection(DbConnectionStringBuilder builder)
    {
        ...
        var sqliteBuilder = builder as SqliteConnectionStringBuilder ??
                            new SqliteConnectionStringBuilder(builder.ConnectionString);
    ...
    /// <remarks>
    ///     <para>
    ///         SQLite doesn't use username/password authentication. These parameters are accepted for API compatibility but
    ///         ignored.
    ///     </para>
    ///     <para>
    ///         For SQLite, server and database are the same concept (file path). We prefer the server parameter to maintain
    ///         consistency with the Name property, which uses ServerKeyName ("Data Source").
    ///     </para>
    /// </remarks>
