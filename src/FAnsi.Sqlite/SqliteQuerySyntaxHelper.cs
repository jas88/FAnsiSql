...
/// <remarks>
///     <para>
///         SQLite uses square brackets [] for identifier quoting and has different function names
///         compared to other databases. It also uses LIMIT for TOP queries.
///     </para>
...
public sealed class SqliteQuerySyntaxHelper : QuerySyntaxHelper
{
    ...
    private SqliteQuerySyntaxHelper() : base(SqliteTypeTranslater.Instance, SqliteAggregateHelper.Instance,
        SqliteUpdateHelper.Instance, DatabaseType.Sqlite)
    {
    }
    ...
    public override string EnsureWrappedImpl(string databaseOrTableName) =>
        $"\"{GetRuntimeNameWithEscapedQuotes(databaseOrTableName)}\"";

    /// <summary>
    ///     Returns the runtime name of the string with all double quotes escaped (but resulting string is not wrapped in
    ///     quotes itself).
    /// </summary>
    ...
    ///     <list type="bullet">
    ///         <item>
    ///             <description>GetTodaysDate: date('now')</description>
    ///         </item>
    ///         <item>
    ///             <description>GetGuid: lower(hex(randomblob(16))) - pseudo-UUID</description>
    ///         </item>
    ///         <item>
