...
/// <summary>
///     Determines the behaviour of <see cref="IDiscoveredDatabaseHelper.CreateTable" />.  This includes how columns are
///     assigned data types, whether foreign keys
///     are created etc.
/// </summary>
/// <remarks>
...
public sealed class CreateTableArgs(DiscoveredDatabase database, string tableName, string? schema)
{
    ...
    /// <summary>
    ///     Create a table with the given name based on the columns and data in the provided <paramref name="dataTable" />.  If
    ...
    /// </summary>
    public CreateTableArgs(DiscoveredDatabase database, string tableName, string schema, DataTable dataTable,
        bool createEmpty, Dictionary<DatabaseColumnRequest, DiscoveredColumn> foreignKeyPairs, bool cascadeDelete)
    ...
    /// <summary>
    ///     Schema of the <see cref="Database" /> to create the table in.  This is NOT the database e.g. in
    ///     [MyDb].[dbo].[MyTable] the schema is "dbo". If in doubt leave blank
    /// </summary>
    public string? Schema { get; private set; } = schema;

    /// <summary>
    ///     Optional - Columns are normally created based on supplied DataTable data rows.  If this is set then the Type
    ///     specified here will
    ///     be used instead.
    /// </summary>
    ...
    /// <summary>
    ///     Link between columns that you want to create in your table <see cref="DatabaseColumnRequest" /> and existing
    ///     columns (<see cref="DiscoveredColumn" />) that
    ...
    /// <summary>
    ///     When creating a foreign key constraint (See <see cref="ForeignKeyPairs" />) determines whether ON DELETE CASCADE
    ///     should be set.
    /// </summary>
    public bool CascadeDelete { get; set; }

    /// <summary>
    ///     The data to use to determine table schema and load into the newly created table (unless <see cref="CreateEmpty" />
    ///     is set).
    /// </summary>
    ...
    /// <summary>
    ///     Populated after the table has been created (See <see cref="TableCreated" />), list of the <see cref="Guesser" />
    ///     used to create the columns in the table.
    ...
    /// <summary>
    ///     Used to determine what how to parse untyped strings in <see cref="DataTable" /> (if building schema from data
    ///     table).
    /// </summary>
    public CultureInfo Culture { get; set; } = CultureInfo.CurrentCulture;

    /// <summary>
    ///     Declare that the table has been created and the provided <paramref name="columnsCreated" /> were used to determine
    ///     the column schema
    /// </summary>
