...
/// <summary>
///     Request to create a column in a DatabaseType agnostic manner.  This class exists to let you declare a field called
///     X where the data type is wide enough
///     to store strings up to 10 characters long (For example) without having to worry that it is varchar(10) in SqlServer
///     but varchar2(10) in Oracle.
///     <para>
///         Type specification is defined in the DatabaseTypeRequest but can also be specified explicitly (e.g.
///         'varchar(10)').
///     </para>
/// </summary>
...
{
    public DatabaseColumnRequest(string columnName, string explicitDbType, bool allowNulls = true) : this(columnName,
        (DatabaseTypeRequest?)null, allowNulls)
    {
    ...
    /// <summary>
    ///     Applies only if the <see cref="TypeRequested" /> is string based.  Setting this will override the default collation
    ///     and specify
    ...
    /// <summary>
    ///     Returns <see cref="ExplicitDbType" /> if set or uses the <see cref="TypeTranslater" /> to generate a proprietary
    ///     type name for <see cref="TypeRequested" />
    /// </summary>
