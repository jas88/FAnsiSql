...
/// <summary>
///     Cross database type functionality for helping build SQL commands that will work regardless of DatabaseType
///     (Microsoft Sql Server / MySql etc).  Describes
///     how to translate broad requirements like 'database type capable of storing strings up to 10 characters long' into a
///     specific implementation e.g.
///     'varchar(10)' in Microsoft SQL Server and 'varchar2(10)' in Oracle (See ITypeTranslater).
///     <para>
///         Also includes features such as qualifying database entities [MyDatabase]..[MyTable].[MyColumn] in Sql Server vs
///         `MyDatabase`.`MyTable`.`MyColumn` in
///         MySql.
///     </para>
...
public interface IQuerySyntaxHelper
{
    ...
    /// <summary>
    ///     The character that is used to end qualifying database entity names e.g. "]" for "[My Table]".  For some DBMS this
    ///     is the same as <see cref="OpenQualifier" />
    /// </summary>
    ...
    /// <summary>
    ///     Creates parameters names from the column names in the collection.  Use this for INSERT etc commands to avoid SQL
    ///     injection and handle creepy column
    ///     names with spaces or reserved keywords
    /// </summary>
    /// <param name="columns"></param>
    /// <param name="toStringFunc">
    ///     Function to convert the <typeparamref name="T" /> to a string e.g. c.ColumnName if
    ///     DataColumn
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     True if the DBMS supports SQL declared parameters (e.g. "DECLARE @bob varchar(10)") whose values can be changed in
    ///     SQL.  False if the only way to
    ...
    /// <summary>
    ///     Ensures that the supplied single entity object e.g. "mytable" , "mydatabase, "[mydatabase]", "`mydatabase` etc is
    ///     returned wrapped in appropriate qualifiers for
    ...
    string EnsureFullyQualified(string? databaseName, string? schemaName, string tableName, string columnName,
    ...
    /// <summary>
    ///     The SQL that would be valid for a CREATE TABLE statement that would result in a given column becoming auto
    ///     increment e.g. "IDENTITY(1,1)"
    /// </summary>
    /// <returns></returns>
    string GetAutoIncrementKeywordIfAny();

    /// <summary>
    ///     Get a list of functions to SQL code (including parameter names).  This is used primarily in autocomplete situations
    ...
    /// <summary>
    ///     Returns SQL that will wrap a single line of SQL with the SQL to calculate MD5 hash e.g. change `MyTable`.`MyColumn`
    ///     to md5(`MyTable`.`MyColumn`)
    ...
    /// <summary>
    ///     Gets a DbParameter hard typed with the correct DbType for the discoveredColumn and the Value set to the correct
    ///     Value representation (e.g. DBNull for nulls or whitespace).
    ///     <para>Also handles converting DateTime representations since many DBMS are a bit rubbish at that</para>
    /// </summary>
    /// <param name="p"></param>
    /// <param name="discoveredColumn">
    ///     The column the parameter is for loading - this is used to determine the DbType for the
    ///     paramter
    /// </param>
    /// <param name="value">
    ///     The value to populate into the command, this will be converted to DBNull.Value if the value is
    ///     nullish
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Throws <see cref="RuntimeNameException" /> if the supplied name is invalid (because it is too long or contains
