...
/// <summary>
///     Cross database type reference to a specific database on a database server.  Allows you to create tables, drop check
///     existance etc.
/// </summary>
public sealed class DiscoveredDatabase : IHasRuntimeName, IMightNotExist
{
    // Cached CompositeFormat for CA1863
    private static readonly CompositeFormat DatabaseDoesNotExistFormat =
        CompositeFormat.Parse(FAnsiStrings.DiscoveredDatabase_DatabaseDoesNotExistSoCannotBeDropped);
    ...
    ///     Database level operations are usually not transaction bound so be very careful about setting
    ///     a parameter for this
    /// </param>
    /// <returns></returns>
    public bool Exists(IManagedTransaction? transaction = null) => Server.Helper.DatabaseExists(this);

    /// <summary>
    ///     Connects to the server and returns a list of tables/views found as <see cref="DiscoveredTable" />.  If you know
    ///     your table exists and you only want to find one you
    ...
    /// <param name="transaction">
    ///     Optional - if provided the database query will be sent using the connection/transaction
    ///     provided
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Connects to the server and returns a list of table valued functions found as
    ///     <see cref="DiscoveredTableValuedFunction" />.  If you know your function exists and you
    ...
    /// </param>
    /// <returns></returns>
    public IEnumerable<DiscoveredTableValuedFunction> DiscoverTableValuedFunctions(
    ...
    /// <summary>
    ///     Returns the wrapped e.g. "[MyDatabase]" name of the database including escaping e.g. if you wanted to name a
    ///     database "][nquisitor" (which would return "[]][nquisitor]").
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     <para>
    ///         Creates an expectation (See <see cref="IMightNotExist" />) that there is a table with the given name in the
    ///         database.
    ///         This method does not query the database or confirm it exists.
    ///     </para>
    ...
    /// <param name="schema">
    ///     Optional - The schema (if supported by DBMS) it exists in.  This is NOT the database e.g. in [MyDb].[dbo].[MyTable]
    ///     the schema is "dbo".
    ...
    public DiscoveredTableValuedFunction ExpectTableValuedFunction(string tableName, string? schema = null) =>
        new(this, tableName, _querySyntaxHelper, schema);

    /// <summary>
    ///     Connects to the database and returns a list of stored proceedures found as <see cref="DiscoveredStoredprocedure" />
    ///     objects
    /// </summary>
    /// <returns></returns>
    public IEnumerable<DiscoveredStoredprocedure> DiscoverStoredprocedures() =>
        Helper.ListStoredprocedures(Server.Builder, GetRuntimeName());
    ...
    /// <summary>
    ///     Drops the database from the server (deletes).  There is no going back after calling this method unless you have a
    ///     database backup.
    /// </summary>
    public void Drop()
    {
        if (!Exists())
            throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture, DatabaseDoesNotExistFormat,
                this));
    ...
    /// <param name="schema">
    ///     Optional - The schema (if supported by DBMS) to create in.  This is NOT the database e.g. in [MyDb].[dbo].[MyTable]
    ///     the schema is "dbo".
    ///     If in doubt leave blank
    /// </param>
    /// <param name="adjuster">
    ///     Last minute delegate class for modifying the <paramref name="columns" /> data types prior to
    ///     executing SQL
    /// </param>
    ...
    ///     Creates a single foreign key between the table created (parent) and a child table.  Columns in this parameter
    ...
    ///     True to set CASCADE DELETE on the foreign key created by
    ///     <paramref name="foreignKeyPairs" />
    /// </param>
    /// <returns>The table created</returns>
    ...
    /// <summary>
    ///     <para>
    ///         Assembles and runs a CREATE TABLE sql statement based on the data/columns in <paramref name="dt" /> and
    ///         returns the table created as a <see cref="DiscoveredTable" />.
    ///     </para>
    ///     <para>
    ///         This will also INSERT the data in <paramref name="dt" /> into the table created unless
    ///         <paramref name="createEmpty" /> is true
    ///     </para>
    /// </summary>
    /// <param name="tableName">The unqualified name for the table you want to create e.g. "MyTable"</param>
    /// <param name="dt">
    ///     Data on which to base the CREATE statement on.  Supports untyped (string) data e.g. "1", "101" "200"
    ///     would create an int column
    /// </param>
    /// <param name="createEmpty">
    ///     True to bulk insert the Rows in the <paramref name="dt" /> after issuing CREATE.  False to
    ///     create only the empty schema
    /// </param>
    /// <param name="adjuster">Last minute delegate class for modifying the table columns data types prior to executing SQL</param>
    /// <param name="explicitColumnDefinitions">
    ///     Optional - Override descisions made about columns in the <paramref name="dt" />
    ///     by specify an explicit type etc
    /// </param>
    /// <returns>The table created</returns>
    public DiscoveredTable CreateTable(string tableName, DataTable dt,
        DatabaseColumnRequest[]? explicitColumnDefinitions = null, bool createEmpty = false,
        IDatabaseColumnRequestAdjuster? adjuster = null) =>
    ...
    /// <returns></returns>
    public DiscoveredTable CreateTable(out Dictionary<string, Guesser>? typeDictionary, string tableName, DataTable dt,
        DatabaseColumnRequest[]? explicitColumnDefinitions = null, bool createEmpty = false,
        IDatabaseColumnRequestAdjuster? adjuster = null)
    {
        ...
        if (!args.TableCreated)
            throw new InvalidOperationException(FAnsiStrings
                .DiscoveredDatabase_CreateTableDidNotPopulateTableCreatedProperty);
    ...
    /// <summary>
    ///     <para>
    ///         Detach this DiscoveredDatabase and returns the data path where the files are stored (local to the DBMS
    ///         server).
    ///     </para>
    ...
    /// <summary>
    ///     Creates a local (to the DBMS server) backup of the database.  Implementations may vary but should be the simplest
    ///     database type
    ...
    /// <returns></returns>
    private bool Equals(DiscoveredDatabase other) => Equals(Server, other.Server) &&
                                                     string.Equals(_database, other._database,
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            return ((Server != null ? Server.GetHashCode() : 0) * 397) ^
                   (_database != null ? StringComparer.OrdinalIgnoreCase.GetHashCode(_database) : 0);
        }
    }
}
