...
/// <summary>
///     Cross database type reference to a Table (or view) in a Database.  Use TableType to determine whether it is a view
///     or a table.  Allows you to check
...
public class DiscoveredTable : IHasFullyQualifiedNameToo, IMightNotExist, IHasQuerySyntaxHelper,
    IEquatable<DiscoveredTable>
{
    // Cached CompositeFormats for CA1863
    private static readonly CompositeFormat DiscoverColumnFailedFormat = CompositeFormat.Parse(FAnsiStrings
        .DiscoveredTable_DiscoverColumn_DiscoverColumn_failed__could_not_find_column_called___0___in_table___1__);

    private static readonly CompositeFormat InsertFailedFormat = CompositeFormat.Parse(FAnsiStrings
        .DiscoveredTable_Insert_Insert_failed__could_not_find_column_called___0___in_table___1__);
    ...
    /// <summary>
    ///     Helper for generating queries compatible with the DBMS the table exists in (e.g. TOP X, column qualifiers, what the
    ///     parameter symbol is etc).
    /// </summary>
    protected readonly IQuerySyntaxHelper QuerySyntaxHelper;

    /// <summary>
    ///     <para>
    ///         Schema of the <see cref="Database" /> the table exists in (or null).  This is NOT the database e.g. in
    ///         [MyDb].[dbo].[MyTable] the schema is "dbo".
    ///     </para>
    ...
    /// <summary>
    ///     Whether the table referenced is a normal table, view or table valued function (see derived class
    ///     <see cref="DiscoveredTableValuedFunction" />)
    /// </summary>
    ...
    ///     <see cref="DiscoveredDatabase.ExpectTable" /> instead.
    /// </summary>
    ...
    public DiscoveredTable(DiscoveredDatabase database, string table, IQuerySyntaxHelper querySyntaxHelper,
        string? schema = null, TableType tableType = TableType.Table)
    {
        TableName = table;
    ...
    public bool Equals(DiscoveredTable? other)
    {
        ...
        return
            ...
            && GetSchemaWithDefaultForNull().AsSpan().Equals(other.GetSchemaWithDefaultForNull().AsSpan(),
    ...
    /// <summary>
    ///     Returns the fully qualified (including schema if appropriate) name of the table e.g. [MyDb].dbo.[MyTable] or
    ///     `MyDb`.`MyTable`
    /// </summary>
    /// <returns></returns>
    public virtual string GetFullyQualifiedName() =>
        QuerySyntaxHelper.EnsureFullyQualified(Database.GetRuntimeName(), Schema, GetRuntimeName());

    /// <summary>
    ///     Gets helper for generating queries compatible with the DBMS the table exists in (e.g. TOP X, column qualifiers,
    ///     what the parameter symbol is etc).
    /// </summary>
    /// <returns></returns>
    ...
    /// <summary>
    ///     <para>
    ///         Checks that the <see cref="Database" /> exists then lists the tables in the database to confirm this table
    ///         exists on the server
    ///     </para>
    /// </summary>
    /// <param name="transaction">
    ///     Optional - if set the connection to list tables will be sent on the connection on which the current
    ///     <paramref name="transaction" /> is open
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Returns the wrapped e.g. "[MyTbl]" name of the table including escaping e.g. if you wanted to name a table
    ...
    ///     Optional - if set the connection to list tables will be sent on the connection on which the current
    ///     <paramref name="managedTransaction" /> is open
    /// </param>
    /// <returns></returns>
    ...
    /// <summary>
    ///     Returns from <see cref="DiscoverColumns" /> the <paramref name="specificColumnName" /> on the server.  This is not
    ///     not case sensitive.  Requires
    ...
    /// <param name="enforceTypesAndNullness">
    ///     True to set <see cref="DataColumn" /> constraints on the <see cref="DataTable" /> returned e.g. AllowDBNull based
    ///     on the table
    ///     schema of the <see cref="DiscoveredTable" />
    /// </param>
    /// <param name="transaction">
    ///     Optional - if set the connection to fetch the data will be sent on the connection on which
    ///     the current <paramref name="transaction" /> is open
    /// </param>
    /// <returns></returns>
    public DataTable GetDataTable(int topX = int.MaxValue, bool enforceTypesAndNullness = true,
        IManagedTransaction? transaction = null) =>
        GetDataTable(new DatabaseOperationArgs { TransactionIfAny = transaction }, topX, enforceTypesAndNullness);

    public DataTable GetDataTable(DatabaseOperationArgs args, int topX = int.MaxValue,
        bool enforceTypesAndNullness = true)
    {
    ...
    public int GetRowCount(IManagedTransaction? transaction = null) =>
        GetRowCount(new DatabaseOperationArgs { TransactionIfAny = transaction });
    ...
    /// </summary>
    /// <param name="transaction">
    ///     Optional - if set the query will be sent on the connection on which the current
    ///     <paramref name="transaction" /> is open
    /// </param>
    /// <returns></returns>
    public bool IsEmpty(IManagedTransaction? transaction = null) =>
        IsEmpty(new DatabaseOperationArgs { TransactionIfAny = transaction });
    ...
    ///     The length of time to wait in seconds before giving up (See
    ...
    {
        AddColumn(name, Database.Server.GetQuerySyntaxHelper().TypeTranslater.GetSQLDBTypeForCSharpType(type),
            allowNulls, args);
    }
    ...
    /// <summary>
    ///     Creates a new object for bulk inserting records into the table.  You should use a using block since
    ///     <see cref="IBulkCopy" /> is <see cref="IDisposable" />.
    ///     Depending on implementation, records may not be committed to the server until the <see cref="IBulkCopy" /> is
    ///     disposed.
    /// </summary>
    ...
    public IBulkCopy BeginBulkInsert(IManagedTransaction? transaction = null) =>
        BeginBulkInsert(CultureInfo.CurrentCulture, transaction);
    ...
    /// <summary>
    ///     Creates and runs a TRUNCATE TABLE SQL statement to delete all rows from the table.  Depending on DBMS and table
    ///     constraints this might fail (e.g. if there are
    ...
    /// <summary>
    ///     Deletes all EXACT duplicate rows from the table leaving only unique records.  This is method may not be
    ///     transaction/threadsafe
    /// </summary>
    /// <param name="timeoutInSeconds">
    ///     The length of time to allow for the command to complete (See
    ///     <see cref="DbCommand.CommandTimeout" />)
    /// </param>
    ...
    /// <summary>
    ///     <para>
    ///         Scripts the table columns, optionally adjusting for nullability / identity etc.  Optionally translates the SQL
    ///         to run and create a table in a different
    ///         database / database language / table name
    ///     </para>
    ...
    /// <param name="convertIdentityToInt">
    ///     True if the resulting script should replace identity columns with int in the
    ///     generated SQL
    /// </param>
    /// <param name="toCreateTable">
    ///     Optional, If provided the SQL generated will be adjusted to create the alternate table instead (which could include
    ///     going cross server type e.g. MySql to Sql Server)
    ///     <para>
    ///         When using this parameter the table must not exist yet, use
    ///         destinationDiscoveredDatabase.ExpectTable("MyYetToExistTable")
    ///     </para>
    /// </param>
    /// <returns></returns>
    public string ScriptTableCreation(bool dropPrimaryKeys, bool dropNullability, bool convertIdentityToInt,
        DiscoveredTable? toCreateTable = null) => Helper.ScriptTableCreation(this, dropPrimaryKeys, dropNullability,
        convertIdentityToInt, toCreateTable);
    ...
    /// <param name="timeoutInSeconds">The number of seconds to wait for the operation to complete</param>
    /// <param name="discoverColumns">Columns that should become part of the primary key</param>
    public void CreatePrimaryKey(IManagedTransaction? transaction, CancellationToken token, int timeoutInSeconds,
        params DiscoveredColumn[] discoverColumns)
    {
    ...
    /// <param name="discoverColumns"></param>
    /// <param name="isUnique"></param>
    public void CreateIndex(DatabaseOperationArgs args, string indexName, DiscoveredColumn[] discoverColumns,
        bool isUnique = false)
    {
    ...
    /// <summary>
    ///     Inserts the values specified into the database table and returns the last autonum identity generated (or 0 if none
    ///     present)
    /// </summary>
    ...
    public int Insert(Dictionary<DiscoveredColumn, object> toInsert, IManagedTransaction? transaction = null) =>
        Insert(toInsert, null, transaction);
    ...
    /// <param name="transaction"></param>
    /// <returns></returns>
    public int Insert(Dictionary<DiscoveredColumn, object> toInsert, CultureInfo? culture,
        IManagedTransaction? transaction = null)
    {
        ...
        var _parameterNames =
            syntaxHelper.GetParameterNamesFor(toInsert.Keys.ToArray(), static c => c.GetRuntimeName());
    ...
    /// <param name="transaction">ongoing transaction this insert should be part of</param>
    /// <returns></returns>
    public int Insert(Dictionary<string, object> toInsert, CultureInfo? culture,
        IManagedTransaction? transaction = null)
    {
    ...
    public DbCommand GetCommand(string s, DbConnection con, DbTransaction? transaction = null) =>
        Database.Server.Helper.GetCommand(s, con, transaction);
    ...
    public DiscoveredRelationship AddForeignKey(DiscoveredColumn foreignKey, DiscoveredColumn primaryKey,
        bool cascadeDeletes, string? constraintName = null, DatabaseOperationArgs? args = null) => AddForeignKey(
        new Dictionary<DiscoveredColumn, DiscoveredColumn> { { foreignKey, primaryKey } }, cascadeDeletes,
        constraintName, args);
