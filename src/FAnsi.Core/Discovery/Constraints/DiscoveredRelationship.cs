...
///     <see cref="DiscoveredTable.DiscoverRelationships" /> instead.
/// </remarks>
...
public sealed class DiscoveredRelationship(
    string fkName,
    DiscoveredTable pkTable,
    DiscoveredTable fkTable,
    CascadeRule deleteRule)
{
    ...
    /// <summary>
    ///     Mapping of primary key column(s) in <see cref="PrimaryKeyTable" /> to foreign key column(s) in
    ///     <see cref="ForeignKeyTable" />.  If there are more than one entry
    ///     then the foreign key is a composite key.
    /// </summary>
    public Dictionary<DiscoveredColumn, DiscoveredColumn> Keys { get; } = [];

    /// <summary>
    ///     Describes what happens to records in the <see cref="ForeignKeyTable" /> when thier parent records (in the
    ///     <see cref="PrimaryKeyTable" />) are deleted.
    /// </summary>
    public CascadeRule CascadeDelete { get; private set; } = deleteRule;

    /// <summary>
    ///     Discovers and adds the provided pair to <see cref="Keys" />.  Column names must be members of
    ///     <see cref="PrimaryKeyTable" /> and <see cref="ForeignKeyTable" /> (respectively)
    /// </summary>
