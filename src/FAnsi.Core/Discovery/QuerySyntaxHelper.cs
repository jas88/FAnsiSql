...
public abstract partial class QuerySyntaxHelper(
    ...
    : IQuerySyntaxHelper
{
    private static readonly SearchValues<char> BracketSearcher = SearchValues.Create("()");

    // Cached CompositeFormats for CA1863
    private static readonly CompositeFormat EnsureWrappedContainsSeparatorsFormat =
        CompositeFormat.Parse(FAnsiStrings
            .QuerySyntaxHelper_EnsureWrapped_String_passed_to_EnsureWrapped___0___contained_separators__not_allowed____Prohibited_Separator_is___1__);

    private static readonly CompositeFormat SplitLineMultipleAliasesFormat =
        CompositeFormat.Parse(FAnsiStrings.QuerySyntaxHelper_SplitLineIntoSelectSQLAndAlias_);

    private static readonly CompositeFormat GetParameterCouldNotFormat =
        CompositeFormat.Parse(FAnsiStrings.QuerySyntaxHelper_GetParameter_Could_not_GetParameter_for_column___0__);

    private static readonly CompositeFormat ValidateNameBlankFormat =
        CompositeFormat.Parse(FAnsiStrings.QuerySyntaxHelper_ValidateName__0__name_cannot_be_blank);

    private static readonly CompositeFormat ValidateNameTooLongFormat = CompositeFormat.Parse(FAnsiStrings
        .QuerySyntaxHelper_ValidateName__0__name___1___is_too_long_for_the_DBMS___2__supports_maximum_length_of__3__);
    ...
    public ITypeTranslater TypeTranslater { get; } = translater;

    public IAggregateHelper AggregateHelper { get; } = aggregateHelper;
    ...
    public virtual string? GetRuntimeName(string? s)
    {
        ...
        if (lastWord.StartsWith(OpenQualifier, StringComparison.Ordinal) &&
            lastWord.EndsWith(CloseQualifier, StringComparison.Ordinal))
    ...
    public string? EnsureWrapped(string? databaseOrTableName)
    {
        ...
        if (databaseOrTableName.Contains(DatabaseTableSeparator))
            throw new ArgumentException(string.Format(CultureInfo.InvariantCulture,
                EnsureWrappedContainsSeparatorsFormat, databaseOrTableName, DatabaseTableSeparator));
    ...
    public virtual string EnsureFullyQualified(string? databaseName, string? schemaName, string tableName,
        string columnName, bool isTableValuedFunction = false) =>
        isTableValuedFunction
            ? $"{GetRuntimeName(tableName)}.{GetRuntimeName(columnName)}"
    ...
    public virtual string GetParameterDeclaration(string proposedNewParameterName, DatabaseTypeRequest request) =>
        GetParameterDeclaration(proposedNewParameterName, TypeTranslater.GetSQLDBTypeForCSharpType(request));
    ...
    /// <returns></returns>
    public virtual bool SplitLineIntoSelectSQLAndAlias(string lineToSplit, out string selectSQL,
        [NotNullWhen(true)] out string? alias)
    {
        ...
        {
            case > 1:
                throw new SyntaxErrorException(string.Format(CultureInfo.InvariantCulture,
                    SplitLineMultipleAliasesFormat, matches.Count, lineToSplit));
            case 0:
        ...
        CultureInfo? culture)
    {
        try
        {
            ...
            if (IsBasicallyNull(value))
            {
                ...
                (value is string strVal &&
                 factory.IsSupported(cSharpType)) //if the input is a string and it's for a hard type e.g. TimeSpan
            {
        ...
        catch (DbException ex)
        {
            throw new InvalidOperationException(
                string.Format(CultureInfo.InvariantCulture, GetParameterCouldNotFormat,
                    discoveredColumn.GetFullyQualifiedName()), ex);
        }
    ...
    public DbParameter GetParameter(DbParameter p, DiscoveredColumn discoveredColumn, object value) =>
        GetParameter(p, discoveredColumn, value, null);
    ...
    public Dictionary<T, string> GetParameterNamesFor<T>(T[] columns, Func<T, string?> toStringFunc) where T : notnull
    {
        ...
        for (var i = 0; i < columns.Length; i++)
        {
            ...
            if (columnName is null ||
                !sensibleParameterNamesInclude.IsMatch(columnName)) //if column name is "_:_" or something
                toReturn.Add(c, $"{ParameterSymbol}p{i}");
            else
                toReturn.Add(c,
                    ParameterSymbol +
                    (reservedKeywords.Contains(columnName)
                        ? $"{columnName}1"
                        : columnName)); //if column is reserved keyword or normal name
        }
    ...
    public static HashSet<string> GetAllParameterNamesFromQuery(string query)
    {
        ...
        var toReturn =
            new HashSet<string>(ParameterNameRegex.Matches(query).Select(static match => match.Groups[1].Value.Trim()),
    ...
    /// <summary>
    ///     <para>
    ///         Removes qualifiers/escape sequences in the suplied <paramref name="name" />.  This should for example convert
    ///         MySql double backtick escape sequences fi``sh into singles (fi`sh).
    ///     </para>
    ///     <para>
    ///         Method is only called after a successful detection and stripping of <see cref="OpenQualifier" /> and
    ///         <see cref="CloseQualifier" />
    ///     </para>
    /// </summary>
    ...
    public static string MakeHeaderNameSensible(string header)
    {
        ...
        for (var i = 0; i < sb.Length; i++)
            ...
            if (sb[i] == ' ' && i + 1 < sb.Length && sb[i + 1] >= 'a' &&
                sb[i + 1] <= 'z') //and there is another character
    ...
    /// <summary>
    ///     <para>
    ///         Return the appropriate value such that it can be put into a DbParameter.Value field and be succesfully inserted
    ///         into a
