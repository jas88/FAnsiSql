...
/// <summary>
///     Cross database type reference to a database server.  Allows you to get connections, create commands, list databases
///     etc.
/// </summary>
public sealed class DiscoveredServer : IMightNotExist
{
    // Cached CompositeFormats for CA1863
    private static readonly CompositeFormat CouldNotConnectTimeoutFormat = CompositeFormat.Parse(FAnsiStrings
        .DiscoveredServer_TestConnection_Could_not_connect_to_server___0___after_timeout_of__1__milliseconds_);

    private static readonly CompositeFormat HelperTriedToCreateDatabaseFormat = CompositeFormat.Parse(FAnsiStrings
        .DiscoveredServer_CreateDatabase_Helper___0___tried_to_create_database___1___but_the_database_didn_t_exist_after_the_creation_attempt);
    ...
    /// </summary>
    /// <param name="builder">
    ///     Determines the connection string and <see cref="DatabaseType" /> e.g.
    ///     MySqlConnectionStringBuilder = DatabaseType.MySql
    /// </param>
    ...
    /// <summary>
    ///     <para>
    ///         Creates a new server pointed at the <paramref name="connectionString" /> which should be a server of DBMS
    ///         <paramref name="databaseType" />
    ///     </para>
    ///     <para><see cref="ImplementationManager" /> must have a loaded implementation for the DBMS type</para>
    /// </summary>
    ...
    /// <summary>
    ///     <para>
    ///         Creates a new server pointed at the <paramref name="server" /> which should be a server of DBMS
    ///         <paramref name="databaseType" />
    ///     </para>
    ///     <para><see cref="ImplementationManager" /> must have a loaded implementation for the DBMS type</para>
    /// </summary>
    ...
    /// <exception cref="ImplementationNotFoundException"></exception>
    public DiscoveredServer(string server, string? database, DatabaseType databaseType, string usernameIfAny,
        string passwordIfAny)
    {
        Helper = ImplementationManager.GetImplementation(databaseType).GetServerHelper();
    ...
    /// <summary>
    ///     <para>
    ///         Returns true/false based on <see cref="TestConnection" />.  This method will use the default timeout of
    ///         <see cref="TestConnection" /> (e.g. 3 seconds).
    ///     </para>
    ///     <para>NOTE: Returns false if any Exception is thrown during <see cref="TestConnection" /></para>
    /// </summary>
    /// <param name="transaction">
    ///     Optional - if provided this method returns true (existence cannot be checked mid
    ///     transaction).
    /// </param>
    /// <returns></returns>
    ...
    [UnconditionalSuppressMessage("ReflectionAnalysis", "IL2057",
        Justification = "Type.GetType with assembly-qualified names is safe for known FAnsiSql provider types.")]
        ...
        foreach (var implementationType in implementationTypes)
            try
            {
                ...
                if (type != null)
                {
                    ...
                    if (instance is IImplementation impl)
    ...
    /// <summary>
    ///     Returns a new unopened connection to the server.  Use
    ///     <see cref="GetCommand(string,FAnsi.Connections.IManagedConnection)" /> to start sending
    ///     <see cref="DbCommand" /> without having to cast.
    /// </summary>
    /// <param name="transaction">
    ///     Optional - when provided returns the <see cref="IManagedTransaction.Connection" /> instead of
    ///     opening a new one
    /// </param>
    /// <returns></returns>
    public DbConnection GetConnection(IManagedTransaction? transaction = null) =>
        transaction != null ? transaction.Connection : Helper.GetConnection(Builder);
    ...
    /// <param name="transaction">
    ///     Optional - if provided the <see cref="DbCommand.Transaction" /> will be set to the
    ...
    ///     to the <see cref="DbCommand.Parameters" /> of <paramref name="command" /> and sets it's
    ///     <paramref name="valueForParameter" />
    /// </summary>
    /// <param name="parameterName"></param>
    ...
    /// <summary>
    ///     <para>
    ///         Creates an expectation (See <see cref="IMightNotExist" />) that there is a database with the given name on the
    ///         server.
    ...
    /// <summary>
    ///     <para>
    ///         Attempts to connect to the server giving up after <paramref name="timeoutInSeconds" /> (if supported by
    ///         DBMS).
    ///     </para>
    ///     <para>
    ///         This differs from <see cref="TestConnection" /> in that it specifies the timeout in the connection string (if
    ///         possible) and waits for the
    ...
    public bool RespondsWithinTime(int timeoutInSeconds, out Exception? exception) =>
        Helper.RespondsWithinTime(Builder, timeoutInSeconds, out exception);
    ...
    /// <summary>
    ///     Edits the connection string (See <see cref="Builder" />) to allow async operations.  Depending on DBMS this may
    ///     have
    ...
    /// <summary>
    ///     <para>
    ///         Edits the connection string (See <see cref="Builder" />) to open connections to the
    ///         <paramref name="newDatabase" />.
    ///     </para>
    ...
    ///     <para>
    ///         In the case of Oracle this is a user+schema (See
    ...
    public DiscoveredDatabase CreateDatabase(string newDatabaseName)
    {
        ...
        if (!db.Exists())
            throw new InvalidOperationException(string.Format(CultureInfo.InvariantCulture,
                HelperTriedToCreateDatabaseFormat, Helper.GetType().Name, newDatabaseName));

        return db;
    }

    /// <summary>
    ///     Opens a new <see cref="DbConnection" /> to the server and starts a <see cref="DbTransaction" />.  These are
    ///     packaged into an <see cref="IManagedConnection" /> which
    ...
    public IManagedConnection BeginNewTransactedConnection() =>
        new ManagedConnection(this, Helper.BeginTransaction(Builder)) { CloseOnDispose = true };

    /// <summary>
    ///     <para>Opens a new <see cref="DbConnection" /> or reuses a thread-local pooled connection.</para>
    ///     <para>
    ///         When <paramref name="transaction" /> is null, returns a pooled connection that is reused across calls on the
    ///         same thread.
    ///         When <paramref name="transaction" /> is provided, bypasses pooling and returns a transacted connection.
    ///     </para>
    ...
    public IManagedConnection GetManagedConnection(IManagedTransaction? transaction = null) =>
        ManagedConnectionPool.GetPooledConnection(this, transaction);

    /// <summary>
    ///     Returns helper for generating queries compatible with the DBMS (See <see cref="DatabaseType" />) e.g. TOP X, column
    ///     qualifiers, what the parameter
    ///     symbol is etc.
    /// </summary>
    /// <returns></returns>
    ...
    private bool Equals(DiscoveredServer other)
    {
        ...
        return string.Equals(Builder.ConnectionString, other.Builder.ConnectionString,
            StringComparison.OrdinalIgnoreCase) && DatabaseType == other.DatabaseType;
    }
