...
public abstract partial class TypeTranslater : ITypeTranslater
{
    ...
    private static readonly CompositeFormat UnsureSqlTypeFormat = CompositeFormat.Parse(FAnsiStrings
        .TypeTranslater_GetSQLDBTypeForCSharpType_Unsure_what_SQL_type_to_use_for_CSharp_Type___0_____TypeTranslater_was___1__);

    private static readonly CompositeFormat NoCSharpTypeMappingFormat = CompositeFormat.Parse(FAnsiStrings
    ...
    /// <summary>
    ///     The maximum number of characters to declare explicitly in the char type (e.g. varchar(500)) before instead
    ///     declaring the text/varchar(max) etc type
    ///     appropriate to the database engine being targeted
    /// </summary>
    private readonly int _maxStringWidthBeforeMax;

    /// <summary>
    ///     The size to declare string fields when the API user has neglected to supply a length.  This should be high, if you
    ///     want to avoid lots of extra long columns
    ...
    /// <summary>
    /// </summary>
    /// <param name="dateRegex">
    ///     <see cref="DateRegex" />
    /// </param>
    /// <param name="maxStringWidthBeforeMax">
    ///     <see cref="_maxStringWidthBeforeMax" />
    /// </param>
    /// <param name="stringWidthWhenNotSupplied">
    ///     <see cref="_stringWidthWhenNotSupplied" />
    /// </param>
    ...
    public string GetSQLDBTypeForCSharpType(DatabaseTypeRequest request)
    {
        ...
        if (t == typeof(string))
            return request.Unicode ? GetUnicodeStringDataType(request.Width) : GetStringDataType(request.Width);
        ...
        throw new TypeNotMappedException(string.Format(CultureInfo.InvariantCulture, UnsureSqlTypeFormat, t.Name,
            GetType().Name));
    }
    ...
    {
        if (decimalSize == null || decimalSize.IsEmpty) return "decimal(20,10)";
    ...
    protected virtual string GetStringDataTypeImpl(int maxExpectedStringWidth) =>
        $"character varying({maxExpectedStringWidth})";
    ...
    /// <summary>
    ///     Returns true if the <paramref name="sqlType" /> (proprietary DBMS type) is a unicode string type e.g. "nvarchar".
    ...
    private static DecimalSize? ParseDecimalSize(ReadOnlySpan<char> typeSpan)
    {
        ...
        for (var i = 0; i < precisionSpan.Length; i++)
            if (!char.IsDigit(precisionSpan[i]))
                return null;

        for (var i = 0; i < scaleSpan.Length; i++)
    ...
    /// <summary>
    ///     Return the number of characters required to not truncate/lose any data when altering a column from time (e.g. TIME
    ///     etc) to varchar(x).  Return
    ///     x such that the column does not lose integrity.  This is needed when dynamically discovering what size to make a
    ///     column by streaming data into a table.
    ///     if we see many times and nulls we will decide to use a time column then we see strings and have to convert the
    ///     column to a varchar column without loosing the
...
select LEN(dt) from omgTimes
    ...
    /// <summary>
    ///     Return the number of characters required to not truncate/loose any data when altering a column from datetime (e.g.
    ///     datetime2, DATE etc) to varchar(x).  Return
    ///     x such that the column does not lose integrity.  This is needed when dynamically discovering what size to make a
    ///     column by streaming data into a table.
    ///     if we see many dates and nulls we will decide to use a date column then we see strings and have to convert the
    ///     column to a varchar column without loosing the
    ///     currently loaded data.
    /// </summary>
    /// <returns></returns>
...
select LEN(dt) from omgdates
    ...
    [GeneratedRegex("^(bit)|(bool)|(boolean)",
    ...
    [GeneratedRegex("^uniqueidentifier",
        RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.CultureInvariant)]
    ...
    [GeneratedRegex("^(float)|(decimal)|(numeric)|(real)|(money)|(smallmoney)|(double)",
        RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.CultureInvariant)]
