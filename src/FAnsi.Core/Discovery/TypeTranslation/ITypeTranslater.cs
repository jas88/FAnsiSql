...
/// <summary>
///     Cross database type functionality for translating between database proprietary datatypes e.g. varchar (varchar2 in
///     Oracle) and the C# Type (and vice
///     versa).
///     <para>
///         When translating into a database type from a C# Type you also need to know additonal information e.g. how long
///         is the maximum length of a string, how much
...
{
    /// <summary>
    ///     DatabaseTypeRequest is turned into the proprietary string e.g. A DatabaseTypeRequest with CSharpType =
    ///     typeof(DateTime) is translated into
    ...
    /// <summary>
    ///     Returns the System.Data.DbType (e.g. DbType.String) for the specified proprietary database type (e.g.
    ///     "varchar(max)")
    /// </summary>
    ...
    ///     for testing if a given random string is likely to be accepted by the DBMS.  You should only pass Types that
    ///     actually
    ///     resolve.
    ...
    /// <summary>
    ///     Translates the given sqlType which must be an SQL string compatible with this TypeTranslater e.g. varchar(10) into
    ///     the destination ITypeTranslater
    ///     e.g. Varchar2(10) if destinationTypeTranslater was Oracle.  Even if both this and the destination are the same you
    ///     might find a different datatype
