...
#if MYSQL_TESTS
internal sealed class CrossPlatformTests_MySql : CrossPlatformTestsBase
{
    ...
    [TestCase("int", "-25")]
    [TestCase("boolean", "0")]
    public void TypeConsensusBetweenGuesserAndDiscoveredTableTest(string datatType, string insertValue) =>
        TypeConsensusBetweenGuesserAndDiscoveredTableTest(datatType, insertValue);
    ...
    [TestCase("_-o-_", ":>0<:")]
    public void HorribleDatabaseAndTableNames(string horribleDatabaseName, string horribleTableName) =>
        HorribleDatabaseAndTableNames(horribleDatabaseName, horribleTableName);
